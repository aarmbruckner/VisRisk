...
import SurveyModel, { SurveyModelCollection } from '../../../API/Models/ModelTemplates/Configuration/SurveyConfig/SurveyModel';
...
class SurveyEditor extends Component {
  ...
  setSelectedSurvey = (selectedSurveyOrig,edit,submit) => {
    if(!selectedSurveyOrig)
      return;

   /*  let selectedSurveyDB = SurveyModelCollection.findOne(
      {_id:selectedSurvey._id}, { sort: [['_id', 'asc']]}
    ); */

    let selectedSurvey = new SurveyModel(null,null,null,null,1);
    //selectedSurvey.LoadFromJSONModel(selectedSurveyDB);
    selectedSurvey.LoadFromJSONModel(selectedSurveyOrig);
    ...
    if(submit === true)
    {
      this.submitSurveyNodeEdit(selectedSurvey,selectedSurveyNode);
    }
  }

  setSelectedSurveyNode = (selectedSurveyNode,edit,submit) => {
    this.setState({
      selectedSurveyNode: cloneDeep(selectedSurveyNode)
    }); 
  ...
  submitSurveyNodeEdit = (selectedSurvey,selectedSurveyNode) => {
  ...
  deleteSurvey = (survey) => {
    ...
    {
      selectedSurvey = null;
      if(this.state.surveyDict  &&  (this.state.surveyDict.size>0 || this.state.surveyDict.length >0))
      {
        ...
        <div className="container-fluid seamless" style={{overflow: 'hidden' }} >
            ...
            <div className="form-group row">
              <div className="col-md-6">
                <SurveyEditorTable deleteSurveyCallback =  {(survey)  => this.deleteSurvey(survey)}  setSelectedSurveyCallback =  {(survey,submit)  => this.setSelectedSurvey(survey,false,submit)} selectedSurvey = {this.state.selectedSurvey}  surveyDict = {this.state.surveyDict}/>
              </div>
              <div className="col-md-5">
                <SurveyEditorDetails selectedSurvey = {this.state.selectedSurvey} setSelectedSurveyCallback =  {(survey,submit)  => this.setSelectedSurvey(survey,true,submit)}  surveyDict = {this.state.surveyDict}/>
              </div>
              <div className="col-md-6">
              </div>
            ...
            <div className="form-group row">
              ...
              <div className="col-md-1" disabled={this.state.surveyNodesDisabled}>
                <Button onClick= {()  => this.submitSurveyNodeEdit()} type="button" className="btn btn-outline-secondary w-100 standardElement"><T>common.commonUI.OK</T></Button>
              </div>
              <div className="col-md-1" disabled={this.state.surveyNodesDisabled}>
              ...
              <div className="col-md-2" disabled={this.state.surveyNodesDisabled}>
