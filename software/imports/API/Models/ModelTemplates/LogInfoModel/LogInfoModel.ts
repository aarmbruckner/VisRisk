...
export default class LogInfoModel implements IJSONParseableModel {
	...
	public GetJSONModel()
	{
		let jsonObject = {
			...
			logGraphModelElements:this.logGraphModelElements
		}  
 

	   if(this.gridInfo)
	   {
		   jsonObject.gridInfo = this.gridInfo.GetJSONModel();
	   }

	   if(this.summaryRectangle)
	   {
		   jsonObject.summaryRectangle = this.summaryRectangle.GetJSONModel();
	   }
	   ...
	   if(this.originRectInfos)
	   {
		   ...
		   for (let originRectInfo of this.originRectInfos) {
			newOriginRectInfos.push(originRectInfo.GetJSONModel());
		   };
	   ...
	   if(this.editableRectInfos)
	   {
		   ...
		   for (let originRectInfo of this.editableRectInfos) {
			newEditableRectInfos.push(originRectInfo.GetJSONModel());
		   };
